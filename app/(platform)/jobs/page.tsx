...
import { Briefcase, Plus, Trash2, EyeOff, Eye } from "lucide-react";
    ...
    <div className="page-container">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px" }}>
        <div>
          <h1 style={{ fontSize: "22px", fontWeight: 700, marginBottom: "4px" }}>
            {session.role === "instituto" ? "Todas as vagas" : "Minhas vagas"}
          </h1>
      ...
      {rows.length === 0 ? (
        <div className="card" style={{ textAlign: "center", padding: "64px 24px" }}>
          ...
          <p style={{ fontWeight: 600, marginBottom: "8px" }}>Nenhuma vaga cadastrada</p>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
                    <span style={{ fontWeight: 700, fontSize: "16px" }}>{job.title}</span>
                  ...
                  <div style={{ fontSize: "13px", color: "var(--idl-text-muted)", marginBottom: "8px" }}>
                    {session.role === "instituto" && <span style={{ marginRight: "12px" }}>🏢 {orgName}</span>}
                    <span style={{ marginRight: "12px" }}>📁 {job.department}</span>
                    <span>📍 {job.location}</span>
                    {job.salaryRange && <span style={{ marginLeft: "12px" }}>💰 {job.salaryRange}</span>}
                  </div>
                  <p style={{ fontSize: "14px", lineHeight: "1.5", color: "var(--idl-text)", WebkitLineClamp: 2, overflow: "hidden", display: "-webkit-box", WebkitBoxOrient: "vertical" }}>
                    {job.description}
                  </p>
                </div>
